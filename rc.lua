...
tags = {
   names = { " 一 ", " 二 ", " 三 ", " 四 ", " 五 " },
   layout = { layouts[1], layouts[2], layouts[3], layouts[4], layouts[1] }
}
...
-- Clock
mytextclock = awful.widget.textclock(markup("#FFFFFF", space3 .. "%H:%M:%S" .. space2))
...
mpdwidget = lain.widgets.mpd({
    settings = function ()
        if mpd_now.state == "play" then
            widget:set_markup(" " .. mpd_now.artist
                              .. " - " .. mpd_now.title .. " ")
...
batterywidget:set_widget(mybatwidget)
batterywidget:set_bgimage(beautiful.widget_bg)
...
myvolume = lain.widgets.alsa({
    ...
    widget:set_markup( v_start .. volume_now.level .. v_end )
  end
})
...
mem_widget = lain.widgets.mem({
    settings = function()
        widget:set_markup(space3 .. mem_now.used .. "MB ")
    end
})
...
-- Separators
spacer = wibox.widget.textbox('<span font="Terminus 4"> </span>')
...
globalkeys = awful.util.table.join(
	...
	awful.key({ modkey, altkey    }, "Right",     function () awful.tag.incmwfact( 0.01)      end),
	awful.key({ modkey, altkey    }, "Left",      function () awful.tag.incmwfact(-0.01)      end),
	awful.key({ modkey, altkey    }, "Down",      function () awful.client.incwfact( 0.01)    end),
	awful.key({ modkey, altkey    }, "Up",        function () awful.client.incwfact(-0.01)    end),
    ...
    awful.key({ modkey }, "F6",
        function ()
            os.execute(string.format("amixer -q set %s 5%%+ unmute", myvolume.channel))
            myvolume.update()
        end),
    awful.key({ modkey }, "F5",
        function ()
    ...
    awful.key({ modkey }, "F3",
        function ()
            os.execute(string.format("amixer -q set %s toggle", myvolume.channel))
            myvolume.update()
        end),
    ...
    awful.key({ modkey }, "q", function () awful.util.spawn(file_manager) end),
